...
void FIF::run(Session *session, const string &src, bool is_decoded = false) {
        ...
        if (session->imageCache->empty()) {
            ...
            if (session->imageCache->find(argument) != session->imageCache->end()) {
                ...
                if (session->loglevel >= 2) {
                    *(session->logfile) << "FIF :: Image cache hit. Number of elements: " <<
                    session->imageCache->size() << endl;
                }
            }
                // Cache Miss
            else {
    ...
    if (session->headers.find("HTTP_IF_MODIFIED_SINCE") != session->headers.end()) {
        ...
        if ((*session->image)->timestamp <= t) {
            if (session->loglevel >= 2) {
                ...
                *(session->logfile) << "FIF :: Total command time " << command_timer.getTime() << " microseconds" <<
                endl;
            }
            throw(304);
        }
